...
for i=1:N_Area
   % Area(i).DirM=eye(2*Area(i).N_of_phases_ext);
    ...
    %tmp_M=rand(2*Area(i).N_of_phases_ext,2*Area(i).N_of_phases_ext);
    %Area(i).DirM=tmp_M*tmp_M'+eye(2*Area(i).N_of_phases_ext);
    Area(i).Rank_E=0;
    Area(i).w=1;
end
% Area(8).w=100;
...
if counter>150 || terminate_flag2
...
%     disp('error')
%     Area(i).Rank_E
%     if value(obj)-value(h)<value(h)*0.01
%         Area(i).w=10*Area(i).w;
%     end
    %%%
    terminate_flag2=1;
    for i=1:N_Area
    terminate_flag2 = terminate_flag2 && (Area(i).Rank_E<0.00002);
    end

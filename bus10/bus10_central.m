           ...
           if node(Area(i).node(n)).phase(j)==1 
               % if   node(Area(i).node(n)).P(j) > 0 || Area(i).node(n)==1
                ...
                if(node(Area(i).node(n)).g==1) 
                 ...
                 Area(i).F_sub=[Area(i).F_sub,c_k1_p(j)*trace(Y_k_p_P*Area(i).X)+c_k1_p(j)*node(Area(i).node(n)).P(j)-Area(i).cg(count_g_phase_temp,1)<=0];
%                                 
...
%           Area(i).DirM=value(Dir_M_temp);
        ...
        terminate_flag2=1;
        for i=1:N_Area
        terminate_flag2 = terminate_flag2 && (max([Area.Rank_E])<0.00005);
        end
        %max([Area.Rank_E])
end
    if terminate_flag2 == 1

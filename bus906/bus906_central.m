...
yalmip('clear')
%BUS DATA
bus1000_node_line_data_org
bus1000_node_line_data_sim
bus1000_Y
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%S_base = 0.1*1e6;% MVA
%V_base = 41.6/sqrt(3);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c_k0_p=0;
c_k1_p1=[0.6,0.7,0.5];%[0.6,0.7,0.5];
c_k1_p2=[0.6,0.7,0.5];%[0.6,0.7,0.5];
%c_k1_p1=[0.6,1,0.5]*4;
%c_k1_p2=[0.6,1,0.5]*4;
c_k2_p=0;


% V_k_p_min=0.95*10/sqrt(3);%0.95;
% V_k_p_max=1.1*10/sqrt(3);%1.05;
% P_g_p_min1=0;
% P_g_p_max1=10;
% Q_g_p_min1=0;
% Q_g_p_max1=10;
           ...
           if node(Area(i).node(n)).phase(j)==1 
               % if   node(Area(i).node(n)).P(j) > 0 || Area(i).node(n)==1
                ...
                if node(Area(i).node(n)).g==1
                 ...
                 Area(i).F_sub=[Area(i).F_sub,P_g_p_min-node(Area(i).node(n)).P(j)-trace(Y_k_p_P*Area(i).X)<=1e-5];
                 Area(i).F_sub=[Area(i).F_sub,trace(Y_k_p_P*Area(i).X)-P_g_p_max+node(Area(i).node(n)).P(j)<=0];
                 Area(i).F_sub=[Area(i).F_sub,Q_g_p_min-node(Area(i).node(n)).Q(j)-trace(Y_k_p_Q*Area(i).X)<=1e-5];
                 ...
                 Area(i).F_sub=[Area(i).F_sub,[c_k1_p(j)*trace(Y_k_p_P*Area(i).X)+c_k0_p+c_k1_p(j)*node(Area(i).node(n)).P(j)-Area(i).cg(count_g_phase_temp,1),...
                    ...
                    Area(i).F_sub=[Area(i).F_sub,node(Area(i).node(n)).P(j)+trace(Y_k_p_P*Area(i).X)<=1e-5];
                    Area(i).F_sub=[Area(i).F_sub,node(Area(i).node(n)).P(j)+trace(Y_k_p_P*Area(i).X)>=-1e-5];
                    Area(i).F_sub=[Area(i).F_sub,node(Area(i).node(n)).Q(j)+trace(Y_k_p_Q*Area(i).X)<=1e-5];
                    Area(i).F_sub=[Area(i).F_sub,node(Area(i).node(n)).Q(j)+trace(Y_k_p_Q*Area(i).X)>=-1e-5];
                    Area(i).F_sub=[Area(i).F_sub,V_k_p_min^2-trace(M_k_p*Area(i).X)<=0];
                    Area(i).F_sub=[Area(i).F_sub,trace(M_k_p*Area(i).X)-V_k_p_max^2<=0];
...
%              if    max([Area.Rank_E])>1e-3 || counter<2
%              [U,S,V]=svd(value(Area(i).X));
%              Area(i).DirM=U(:,2:Area(i).N_of_phases_ext)*U(:,2:Area(i).N_of_phases_ext)';
%              else
...
%              end
    ...
    terminate_flag2=1;
    for i=1:N_Area
    terminate_flag2 = terminate_flag2 && (Area(i).Rank_E<1e-5);
    end
...
disp(X_v(1:N_of_phases)/6.0553 +1i*X_v(N_of_phases+1:2*N_of_phases)/6.0553)